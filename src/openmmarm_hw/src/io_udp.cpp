...
IOUDP::IOUDP(const std::string &mcu_ip, int mcu_port, int local_port)
  ...
  std::cout << "[IOUDP] 初始化 UDP 接口: " << mcu_ip << ":" << mcu_port
            << std::endl;
}
...
bool IOUDP::init() {
  ...
  if (bind(socket_fd_, (struct sockaddr *)&local_addr_, sizeof(local_addr_)) <
      0) {
    std::cerr << "[IOUDP] 绑定本地端口失败" << std::endl;
    close(socket_fd_);
    socket_fd_ = -1;
    return false;
  }
  ...
  if (inet_pton(AF_INET, mcu_ip_.c_str(), &mcu_addr_.sin_addr) <= 0) {
    std::cerr << "[IOUDP] 无效的 MCU IP 地址: " << mcu_ip_ << std::endl;
    close(socket_fd_);
    socket_fd_ = -1;
    return false;
  }
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (recvState(&init_state)) {
      is_connected_ = true;
      initialized_ = true;
      std::cout << "[IOUDP] 成功连接到 MCU" << std::endl;
      return true;
    }
    std::cout << "[IOUDP] 等待 MCU 响应... (" << i + 1 << "/10)" << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
  }

  std::cerr << "[IOUDP] 无法连接到 MCU" << std::endl;
  return false;
}

bool IOUDP::sendRecv(const LowLevelCmd *cmd, LowLevelState *state) {
  if (socket_fd_ < 0) {
    return false;
  }

  sendCmd(cmd);
  bool success = recvState(state);
  is_connected_ = success;
  return success;
}

bool IOUDP::isConnected() { return is_connected_; }
...
bool IOUDP::recvState(LowLevelState *state) {
  UDPRecvState pkt{};
  struct sockaddr_in recv_addr;
  socklen_t addr_len = sizeof(recv_addr);

  ssize_t recv_len = recvfrom(socket_fd_, &pkt, sizeof(pkt), 0,
  ...
  if (recv_len < static_cast<ssize_t>(sizeof(pkt))) {
    std::cerr << "[IOUDP] 接收数据包长度不足: " << recv_len << std::endl;
    return false;
  }
  ...
  if (pkt.crc != expected_crc) {
    std::cerr << "[IOUDP] CRC 校验失败" << std::endl;
    return false;
  }
  ...
  for (int i = 0; i < 6; ++i) {
    ...
    state->q[i] = pkt.q[i];
    state->dq[i] = pkt.dq[i];
    state->ddq[i] = pkt.ddq[i];
    state->tau_est[i] = pkt.tau_est[i];
    state->temperature[i] = pkt.temperature[i];
  }
